 ...
 */
namespace Pbmage\AttributeManager\Observer;
...
class CategoryAttributeAddUpdateAfter implements ObserverInterface
{
    ...
    public function execute(\Magento\Framework\Event\Observer $observer)
    {
        ...
        $file = $root_path.'/app/code/Pbmage/AttributeManager/view/adminhtml/ui_component/category_form.xml';
        if (!$this->fileSystem->isExists($file)) {
            $file = $root_path.'/vendor/pbmage/attributemanager/view/adminhtml/ui_component/category_form.xml';
        }
...
<form xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
    ...
    <fieldset name="pbmage_custom_attributes">
        <argument name="data" xsi:type="array">
            <item name="config" xsi:type="array">
                <item name="label" xsi:type="string" translate="true">Custom Attributes (PBMAGE)</item>
...
';
            ...
            //clean the cache
            $this->pbmageHelper->cleanBackendCache();
        }
    }
    ...
    private function getSelectOptionValues($attribute_id)
    {
        $option_xml = '';
        if ($attribute_id) {
            $this->pbmageHelper->getCoreRegistry()->unregister('entity_attribute');
